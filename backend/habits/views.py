...
from ml_recommendations.recommendation import recommend_habits
...
ONBOARDING_QUESTIONS = [
    {
        ...
        'options': [
            ('sport', 'Спорт', {'health': 1.0}),
            ('health', 'Здоровье', {'health': 1.0}),
            # ('reading', 'Чтение', {'learning': 1.0}),
            ('meditation', 'Медитация', {'mindfulness': 1.0}),
            # ('sleep', 'Сон', {'health': 1.0}),
            # ('food', 'Питание', {'health': 1.0}),
            # ('languages', 'Изучение языков', {'learning': 1.0}),
            ('learning', 'Обучение', {'learning': 1.0}),
            ('productivity', 'Продуктивность', {'productivity': 1.0}),
            ('self_development', 'Саморазвитие', {'self_development': 1.0}),
            # ('concentration', 'Концентрация', {'productivity': 1.0}),
        ],
    },
    {
        ...
        'options': [
            ('never', 'Никогда', {'sport': 0.2}),
            ('1-2 times/week', '1–2 раза в неделю', {'sport': 0.4}),
            ('3-5 times/week', '3–5 раз в неделю', {'sport': 0.7}),
            ('every day', 'Каждый день', {'sport': 1.0}),
        ],
    },
    {
        'name': 'difficulties',
        ...
        'is_multi_select': True,
        'options': [
            ('hard_wake', 'Просыпаться вовремя', {'health': 0.8}),
            ('hard_focus', 'Концентрироваться', {'productivity': 0.8}),
            ('hard_train', 'Тренироваться', {'sport': 0.8}),
    ...
    },
    {
        ...
        'is_multi_select': True,
        'options': [
            ('morning', 'Утро', {'morning': 1.0}),
            ('day', 'День', {'day': 1.0}),
            ('evening', 'Вечер', {'evening': 1.0}),
            ('night', 'Ночью', {'night': 1.0}),
        ],
    },
]
...
def onboarding(request):
    ...
    if request.method == 'POST' and request.POST.get('action') == 'skip':
        ...
        messages.info(request, 'Онбординг пропущен. Ты всегда можешь добавить привычки вручную.')
        return redirect('dashboard')
    ...
    if request.method == 'POST' and request.POST.get('action') == 'add':
        ...
        if created_count:
            ...
            messages.info(request, 'Онбординг завершен. Рекомендации можно добавить вручную позже.')
        return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        if missing:
            ...
            try:
                recommendations = recommend_habits(answers)['recommendations']
