...
def generate_insights(user) -> list[UserInsight]:
    ...
    if best_days and best_days[0]['rate'] > 0:
        ...
        new_insights.append(UserInsight.objects.create(
            ...
            message=f'{best["name"]} — самый активный день. Выполнено {best["done"]} из {best["expected"]} отметок.',
        ...
        if weak['rate'] < best['rate']:
            new_insights.append(UserInsight.objects.create(
                ...
                message=f'{weak["name"]} пока требует внимания. Выполнено {weak["done"]} из {weak["expected"]} отметок.',
